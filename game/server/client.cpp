...
void CPointClientCommand::InputCommand(inputdata_t& inputdata)
{
	...
	if (!Q_strcmp(inputdata.value.String(), "map credits_d_linux"))
		inputdata.value.SetString(AllocPooledString("map credits_d"));

	if (!Q_strcmp(inputdata.value.String(), "pov"))
	{
		clientengine->ExecuteClientCmd("r_drawvgui 0; gameui_activate; wait 1; playvideo Amod_OutroVideo; r_drawvgui 1");
		return;
	}
