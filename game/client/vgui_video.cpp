...
CON_COMMAND(playvideo, "Plays a video: <filename> [width height]")
{
	...
	if (!Q_strcmp(movie, "Amod_OutroVideo") && bUseNewEnding)
	...
	if (!Q_strcmp(args.Arg(1), "Amod_OutroVideo"))
	{
		...
		Q_strcpy(command, "gameui_allowescape; gameui_hide; wait 1; ent_fire logic_ending_credits trigger");
	}
...
CON_COMMAND(amod_playvideo, "Plays a video: <filename> <command after finished> [width height]")
{
	...
	char command[512];
	engine->ClientCmd_Unrestricted("gameui_preventescape");
	Q_snprintf(command, sizeof(command), "gameui_allowescape; gameui_hide; wait 1; %s", args.Arg(2));
